...
from graph import ImprovedNeighborFinder
...
def load_real_data(dataName):
    ...
    ngh_finder = ImprovedNeighborFinder(adj_list, uniform=False)
...
def load_train_real_data(UNIFORM):
    ...
    train_real_datasets = ['aves-weaver-social','ca-cit-HepPh','ca-cit-HepTh','CollegeMsg','edit-itwikivoyage','hyccups','ia-digg-reply'
    ,'ia-facebook-wall-wosn-dir','ia-frwikinews-user-edits','ia-movielens-user2tags-10m','mooc_actions','soc-sign-bitcoinalpha','soc-sign-bitcoinotc','sx-askubuntu']
    ...
    for index in range(len(train_real_datasets)):
        ...
        for src, dst, eidx, ts in zip(src_list[index], dst_list[index], g_df.idx.values, ts_list[index]):
            adj_list[dst].append((src, eidx, ts))
        ngh_finder.append(ImprovedNeighborFinder(adj_list, uniform=UNIFORM))
    return src_list, dst_list, ts_list, node_count, node_list, maxTime_list, ngh_finder
